"""Embeddings and local cosine retrieval for the curated rubric corpus."""
...
from config import EMBEDDING_MODEL, MAX_EVIDENCE_CHARS, MAX_RETRIEVED_CHUNKS, MIN_RETRIEVAL_SCORE
...
def rank_matches(query_embedding: list[float], index: EmbeddedCorpus, limit: int = MAX_RETRIEVED_CHUNKS) -> list[RetrievedChunk]:
    if not index.chunks or len(index.chunks) != len(index.embeddings):
        raise RetrievalError("The rubric index is empty or incomplete.")
    matches = [RetrievedChunk(chunk=chunk, score=cosine_similarity(query_embedding, vector)) for chunk, vector in zip(index.chunks, index.embeddings)]
    ...
    if not results or results[0].score < MIN_RETRIEVAL_SCORE:
        raise RetrievalError("I do not have a reliable curated rubric for that role yet. Try a supported role or use a more specific target role.")
    return results


def retrieve_rubrics(client: OpenAI, target_role: str, index: EmbeddedCorpus, limit: int = MAX_RETRIEVED_CHUNKS) -> list[RetrievedChunk]:
    ...
    return rank_matches(embed_texts(client, [query])[0], index, limit)
...
def format_evidence(matches: list[RetrievedChunk]) -> str:
    ...
    for match in matches:
        block = f"[RUBRIC {match.chunk.chunk_id}: {match.chunk.title}]\n{match.chunk.text.strip()}"
