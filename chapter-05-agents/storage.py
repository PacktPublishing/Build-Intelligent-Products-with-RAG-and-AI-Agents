...
ROASTS_COLUMNS = ["email", "created_at", "user_intent", "score", "roast_text"]
...
def save_roast(email: str, user_intent: str, score: str, roast_text: str) -> None:
    """Append one roast record for the given user."""
    ensure_storage()
    email = _normalize_email(email)

    with open(_roasts_path(), "a", newline="", encoding="utf-8") as f:
        csv.writer(f).writerow([email, _now(), user_intent, score, roast_text])
