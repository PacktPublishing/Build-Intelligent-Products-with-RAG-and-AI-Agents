...
- **Chapter 5** grounds ResumeRoast with retrieval-augmented generation (RAG), so its critiques use the product's own role rubrics rather than an unbounded prompt.
- **Chapter 6** adds a capped, read-only agent to the grounded RAG product.
- **Chapter 7** replaces the MVP's temporary infrastructure (CSV files, email-only identification, a free deployment platform) with production infrastructure: Supabase as the durable database (the graduation trigger the Chapter 4 stack commitment already named), real authentication, logging, monitoring, rate limits, and cost controls.
- **Chapter 8** adds what's needed to get the first real users and distribution.
- **Chapter 9** adds feedback capture, prompt versioning, evaluation, and product analytics so iteration is driven by real user behaviour.
- **Chapter 10** takes the product from side project to something built to scale.
